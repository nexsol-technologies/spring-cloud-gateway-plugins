...
import ch.nexsol.gateway.filter.factory.RecaptchaGatewayFilterFactory.RecaptchaResponseV2;
import ch.nexsol.gateway.filter.factory.RecaptchaGatewayFilterFactory.RecaptchaResponseV3;
...
/**
 * Tests for {@link GatewayFilterRuntimeHints}.
 * <p>
 ...
 */
class GatewayFilterRuntimeHintsTests {

	private static final CodeSource MODULE_CLASSES = GatewayFilterRuntimeHints.class.getProtectionDomain()
		.getCodeSource();
	...
	@BeforeEach
	void registerHints() {
		new GatewayFilterRuntimeHints().registerHints(this.hints, getClass().getClassLoader());
	}

	@Test
	void registersTheConfigOfEveryFilterFactoryOfTheModule() {
		List<Class<?>> configClasses = configClassesOf("ch.nexsol.gateway.filter");
		...
		for (Class<?> configClass : configClasses) {
			assertThat(RuntimeHintsPredicates.reflection()
				...
				.as("%s is not registered; add it to GatewayFilterRuntimeHints", configClass.getName())
				.accepts(this.hints);
		}
	}
	...
	void isContributedThroughAotFactories() {
		...
		assertThat(registrars).hasAtLeastOneElementOfType(GatewayFilterRuntimeHints.class);
	}

	@Test
	void registersTheRecaptchaVerdictsGoogleAnswersWith() {
		assertThat(RuntimeHintsPredicates.reflection().onType(RecaptchaResponseV2.class)).accepts(this.hints);
		assertThat(RuntimeHintsPredicates.reflection().onType(RecaptchaResponseV3.class)).accepts(this.hints);
	}
