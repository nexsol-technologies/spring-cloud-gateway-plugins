...
/**
 * Tests for {@link OpenapiHubRuntimeHints}.
 * <p>
 ...
 */
class OpenapiHubRuntimeHintsTests {

	private static final CodeSource MODULE_CLASSES = OpenapiHubRuntimeHints.class.getProtectionDomain().getCodeSource();

	private final RuntimeHints hints = new RuntimeHints();

	@BeforeEach
	void registerHints() {
		new OpenapiHubRuntimeHints().registerHints(this.hints, getClass().getClassLoader());
	}

	@Test
	void registersTheConfigOfEveryFilterFactoryOfTheModule() {
		List<Class<?>> configClasses = configClassesOf("ch.nexsol.gateway.openapi");
		...
		for (Class<?> configClass : configClasses) {
			assertThat(RuntimeHintsPredicates.reflection()
				...
				.as("%s is not registered; add it to OpenapiHubRuntimeHints", configClass.getName())
				.accepts(this.hints);
		}
	}
	...
	void isContributedThroughAotFactories() {
		...
		assertThat(registrars).hasAtLeastOneElementOfType(OpenapiHubRuntimeHints.class);
	}
