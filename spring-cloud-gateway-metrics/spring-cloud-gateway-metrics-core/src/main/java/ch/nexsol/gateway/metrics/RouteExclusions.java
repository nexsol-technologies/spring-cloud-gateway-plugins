...
/**
 * The route ids left out of the figures, compiled once.
 * <p>
 * Shared by every source so a route hidden from the traffic view stays hidden whichever
 * source answers.
 */
public class RouteExclusions {
	...
	/**
	 * Whether the given route is left out of the figures. The whole id must match, so
	 * {@code docs} excludes {@code docs} but not {@code docs-public}.
	 * @param routeId the route id, possibly {@code null} or blank
	 * @return {@code true} when the route must not be reported
	 */
