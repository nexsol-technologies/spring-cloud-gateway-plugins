...
/**
 * Tests for {@link GatewayMetricsRuntimeHints}.
 * <p>
 * The coverage test scans the module rather than listing the readings: one added later is
 * picked up here and fails the build until it is registered, which is the only warning
 * there is &mdash; a missing hint costs nothing on the JVM and only surfaces as an empty
 * payload in a native image.
 */
class GatewayMetricsRuntimeHintsTests {

	private static final CodeSource MODULE_CLASSES = GatewayMetricsRuntimeHints.class.getProtectionDomain()
		.getCodeSource();
	...
	@BeforeEach
	void registerHints() {
		new GatewayMetricsRuntimeHints().registerHints(this.hints, getClass().getClassLoader());
	}

	@Test
	void registersEveryPublicRecordOfTheModule() {
		List<Class<?>> records = recordsOf("ch.nexsol.gateway.metrics");
		assertThat(records).isNotEmpty();
		for (Class<?> record : records) {
			assertThat(RuntimeHintsPredicates.reflection().onType(record))
				.as("%s is not registered; add it to GatewayMetricsRuntimeHints", record.getName())
				.accepts(this.hints);
		}
	}
	...
	void isContributedThroughAotFactories() {
		...
		assertThat(registrars).hasAtLeastOneElementOfType(GatewayMetricsRuntimeHints.class);
	}
