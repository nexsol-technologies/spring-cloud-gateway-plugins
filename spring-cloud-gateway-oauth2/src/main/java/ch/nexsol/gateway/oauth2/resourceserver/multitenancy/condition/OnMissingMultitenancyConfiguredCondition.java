...
public class OnMissingMultitenancyConfiguredCondition extends SpringBootCondition {
	...
	public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
		...
		if (registrations.isEmpty()) {
			return ConditionOutcome.match(message.because("no registered multitenancy clients"));
		}
		return ConditionOutcome.noMatch(message.because(registrations.size() + " clients registered"));
	}
