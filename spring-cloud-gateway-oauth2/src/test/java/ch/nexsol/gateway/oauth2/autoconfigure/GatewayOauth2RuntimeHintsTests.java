...
/**
 * Tests for {@link GatewayOauth2RuntimeHints}.
 * <p>
 ...
 */
class GatewayOauth2RuntimeHintsTests {

	private static final CodeSource MODULE_CLASSES = GatewayOauth2RuntimeHints.class.getProtectionDomain()
		.getCodeSource();
	...
	@BeforeEach
	void registerHints() {
		new GatewayOauth2RuntimeHints().registerHints(this.hints, getClass().getClassLoader());
	}

	@Test
	void registersTheConfigOfEveryFilterFactoryOfTheModule() {
		List<Class<?>> configClasses = configClassesOf("ch.nexsol.gateway.oauth2");
		...
		for (Class<?> configClass : configClasses) {
			assertThat(RuntimeHintsPredicates.reflection()
				...
				.as("%s is not registered; add it to GatewayOauth2RuntimeHints", configClass.getName())
				.accepts(this.hints);
		}
	}
	...
	void isContributedThroughAotFactories() {
		...
		assertThat(registrars).hasAtLeastOneElementOfType(GatewayOauth2RuntimeHints.class);
	}
