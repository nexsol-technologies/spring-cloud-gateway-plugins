...
/**
 * Tests for {@link OpenapiValidationRuntimeHints}.
 * <p>
 * The coverage tests scan rather than list: a factory added later, or an OpenAPI model
 * package the library moves, fails the build here, which is the only warning there is
 * &mdash; a missing hint costs nothing on the JVM and only surfaces as a contract that
 * validates nothing in a native image.
 */
class OpenapiValidationRuntimeHintsTests {

	private static final CodeSource MODULE_CLASSES = OpenapiValidationRuntimeHints.class.getProtectionDomain()
		.getCodeSource();
	...
	@BeforeEach
	void registerHints() {
		new OpenapiValidationRuntimeHints().registerHints(this.hints, getClass().getClassLoader());
	}

	@Test
	void registersTheConfigOfEveryFilterFactoryOfTheModule() {
		List<Class<?>> configClasses = configClassesOf("ch.nexsol.gateway.validation");
		...
		for (Class<?> configClass : configClasses) {
			assertThat(RuntimeHintsPredicates.reflection()
				...
				.as("%s is not registered; add it to OpenapiValidationRuntimeHints", configClass.getName())
				.accepts(this.hints);
		}
	}
	...
	void registersTheOpenapiModelTheContractsAreParsedInto() {
		for (Class<?> modelClass : List.of(OpenAPI.class, Components.class, PathItem.class, Content.class, Schema.class,
				StringSchema.class, ArraySchema.class)) {
			assertThat(RuntimeHintsPredicates.reflection()
				.onType(modelClass)
				.withMemberCategories(MemberCategory.ACCESS_DECLARED_FIELDS, MemberCategory.INVOKE_DECLARED_METHODS,
						MemberCategory.INVOKE_DECLARED_CONSTRUCTORS))
	...
	void isContributedThroughAotFactories() {
		...
		assertThat(registrars).hasAtLeastOneElementOfType(OpenapiValidationRuntimeHints.class);
	}
