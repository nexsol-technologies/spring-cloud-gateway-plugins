...
/**
 * Matches when the route management is configured to expose reading alone.
 * <p>
 * The value is read as the enum it binds to rather than compared as a string, which is
 * what {@code @ConditionalOnProperty} would do: {@code READ_ONLY}, {@code read-only} and
 * {@code read_only} all configure the same thing, and all three have to match.
 */
class OnRouteManagementReadOnlyCondition extends SpringBootCondition {
	...
	public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
		...
		return (access == RoutesDatabaseProperties.Access.READ_ONLY)
				? ConditionOutcome.match(message.because(PROPERTY + " is read-only"))
				: ConditionOutcome.noMatch(message.because(PROPERTY + " is " + access));
	}
