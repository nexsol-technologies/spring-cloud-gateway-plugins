 ...
 * definitions from JSON/YAML files. The blocking file read is offloaded to the bounded
 * elastic scheduler and the result is cached by the superclass.
 */
public class FileRouteDefinitionLocator extends AbstractRefreshableRouteDefinitionLocator {

	private final FileRouteDefinitionLoader loader;

	/**
	 * Creates the locator.
	 * @param loader the loader reading the configured files
	 * @param publisher the publisher used to trigger route refresh events
	 */
	public FileRouteDefinitionLocator(FileRouteDefinitionLoader loader, ApplicationEventPublisher publisher) {
		super(publisher);
		this.loader = loader;
	}
