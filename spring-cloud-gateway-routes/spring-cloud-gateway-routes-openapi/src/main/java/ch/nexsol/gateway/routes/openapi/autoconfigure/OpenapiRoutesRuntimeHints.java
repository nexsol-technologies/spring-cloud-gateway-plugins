...
/**
 * Registers, for a native image, the OpenAPI model classes the contracts are parsed into.
 * <p>
 * The library ships no reachability metadata of its own, and {@code OpenAPIDeserializer}
 * does not build the model by hand throughout: it falls back on
 * {@code ObjectMapper.convertValue}, so the model classes are read reflectively and a
 * contract silently yields no route once the image is built.
 * <p>
 * They are registered by scanning the model package rather than listed one by one: there
 * are eighty of them, a schema subclass per JSON type, and the set grows with every
 * OpenAPI revision the library follows.
 */
