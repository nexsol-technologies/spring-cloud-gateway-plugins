...
Exercises [spring-cloud-gateway-routes-files](../../../spring-cloud-gateway-routes/spring-cloud-gateway-routes-files/README.md)
on port `8207`: route definitions read from JSON and YAML files, the source a GitOps pipeline
reviews and deploys.
...
The two file formats and the two predicate/filter forms — the shorthand `Path=/files/**` and
the object form with named arguments — are all used across those files, since the plugin
accepts them interchangeably.

## Reloading

```console
$ curl http://localhost:8207/watched/get      # 200
```
...
**Only `file:` locations are watched.** A classpath resource is read once, at startup: edit
`src/main/resources/gateway-routes/*.yaml` and nothing happens until the next restart, or
until the sources are re-read on demand:

```console
curl -X POST http://localhost:8207/actuator/refresh
```

That endpoint re-reads every source from scratch, not just the cached snapshot, and it is
what a Spring Cloud Bus `busrefresh` fans out to a whole fleet.
