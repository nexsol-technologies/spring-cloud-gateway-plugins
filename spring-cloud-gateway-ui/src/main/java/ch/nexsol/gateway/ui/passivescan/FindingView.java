 ...
 *
 * @param ruleId the stable rule id shown in the console, e.g. {@code PS-cors}
 ...
 * @param confidence how sure the scanner is
 ...
 * @param reference a URL to learn more
 * @param routeId the route id, or {@code null}
 * @param evidence the observed values
 */
public record FindingView(String ruleId, Instant lastSeen, Instant firstSeen, long count, String severity,
		String confidence, String categoryCode, String categoryTitle, String cwe, String scanner, String method,
		String path, String title, String detail, String remediation, String reference, String routeId,
		Map<String, String> evidence) {

	static FindingView of(AggregatedFinding aggregate) {
		Finding finding = aggregate.finding();
		return new FindingView("PS-" + finding.scannerId(), aggregate.lastSeen(), aggregate.firstSeen(),
				aggregate.count(), finding.severity().name(), finding.confidence().name(), finding.category().getCode(),
				finding.category().getTitle(), finding.cwe(), finding.scannerId(), finding.method(), finding.path(),
				finding.title(), finding.detail(), finding.remediation(), finding.reference(), finding.routeId(),
				finding.evidence());
	}
